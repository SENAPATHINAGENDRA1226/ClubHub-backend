...
async def seed_data():
    async with AsyncSessionLocal() as session:
        ...
        for c_data in committees_data:
            c_res = await session.execute(select(Committee).filter_by(name=c_data["name"]))
            if not c_res.scalars().first():
                committee = Committee(**c_data)
                session.add(committee)
                print(f"Seeded Committee: {c_data['name']}")

        # 3. Seed 3 Sample Events (Upcoming, Current, Past)
        events_data = [
            {
        ...
        for e_data in events_data:
            e_res = await session.execute(select(Event).filter_by(title=e_data["title"]))
